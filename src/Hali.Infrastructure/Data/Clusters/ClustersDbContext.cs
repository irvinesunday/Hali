...
public class ClustersDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<SignalCluster>(e =>
        {
            ...
            e.Property(x => x.SubcategorySlug).HasColumnName("subcategory_slug").HasMaxLength(80);
            e.Property(x => x.DominantConditionSlug).HasColumnName("dominant_condition_slug").HasMaxLength(80);
            e.Property(x => x.State).HasColumnName("state");
            e.Property(x => x.Title).HasColumnName("title").HasMaxLength(240);
            e.Property(x => x.Summary).HasColumnName("summary");
            e.Property(x => x.LocationLabelId).HasColumnName("location_label_id");
            e.Property(x => x.SpatialCellId).HasColumnName("spatial_cell_id").HasMaxLength(80);
            ...
            e.Property(x => x.PossibleRestorationAt).HasColumnName("possible_restoration_at");
            e.Property(x => x.CivisScore).HasColumnName("civis_score").HasPrecision(8, 4);
            e.Property(x => x.Wrab).HasColumnName("wrab").HasPrecision(10, 4);
            e.Property(x => x.Sds).HasColumnName("sds").HasPrecision(10, 4);
            e.Property(x => x.Macf).HasColumnName("macf");
            e.Property(x => x.RawConfirmationCount).HasColumnName("raw_confirmation_count");
            e.Property(x => x.TemporalType).HasColumnName("temporal_type").HasMaxLength(40);
